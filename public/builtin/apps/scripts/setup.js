...
const handleForm = async (form, type) => {
  switch (type) {
    case 'login': {
      const loginData = await auth.signInWithEmailAndPassword(_auth, form.elements.email.value, form.elements.password.value)
        .catch((error) => handleAuthError(error, 0));
      handleAuthLogin(loginData.user);
      break;
    }
    case 'signup': {
      const signupData = await auth.createUserWithEmailAndPassword(_auth, form.elements.email.value, form.elements.password.value)
        .catch((error) => handleAuthError(error, 1));
      handleAuthSignup(signupData.user, form.elements.username.value);
      break;
    }
  }
};
